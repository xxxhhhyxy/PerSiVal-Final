# PerSiVal-Surface-IOS
1. Unity 2021.3.1f1
2. ARFoundation 4.2
3. Work perfectly on Iphone13 pro
4. This project is currently for security&debugging mode only. 
* The user have to initialize it by click on the checkbox step by step to avoid data crush event.
* The only Update function is in GlobalCtrl.cs, to make sure the data go step by step.
5. How to use this repository:

5.1. clone the repo from Github, don't open it

5.2. download the neural network and so on from https://drive.google.com/file/d/1pOu_9HIz49oguZLXtF8IG5kL7AU6irz5/view?usp=sharing

5.3. decompress the "PersivalData.zip" you downloaded in step 4.2, and then copy the folder "PersivalData" and then paste it in Asset——to overwrite the folder with the same name (because it doesn't contain the onnx file).——doing this before you open the Unity Project may help you avoid the object binding steps in Unity editor.

5.4. if the binding between the scripts and the .onnx files is still missing, it is fine, just need to follow the screenshot below:

![Debug console showing the Server Mode status](./ActivationModel.png)
![Debug console showing the Server Mode status](./DeformationModel.png)
...
This project is based on the example projects that use [*AR Foundation 4.2*](https://docs.unity3d.com/Packages/com.unity.xr.arfoundation@4.2/manual/index.html) and demonstrate its functionality with sample assets and components.
